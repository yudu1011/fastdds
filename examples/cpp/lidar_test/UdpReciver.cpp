...

{
  ...
  sender_address_len_ = sizeof(sender_address_);
  ...
  std::cout << "Creat UDPreciver success.  GroupIP: " <<  groupIP << " recive port： "<< port << std::endl;
...
long UdpReciver::run(char *buffer_, int size)
{
  // std::cout << sizeof(buffer_) << std::endl;
  // char buffer[1200];
  // std::cout << sizeof(buffer) << std::endl;
  // *buffer_[0] = 0xff;
  long bytes_received = recvfrom(sock_, buffer_, size, 0,
                                    (struct sockaddr *)&sender_address_, &sender_address_len_);
  if (bytes_received > 0) {
      buffer_[bytes_received] = '\0'; // 确保字符串以空字符结尾
