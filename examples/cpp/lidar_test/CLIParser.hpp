...
public:
            ...
            uint8_t return_code)
    {
        ...
        std::cout << "      --groupIP <str>                 IPv4 for reciving the data from muliti UDP" << std::endl;
        std::cout << "                                      (Default: <224.0.2.3>)"                     << std::endl;
        std::cout << "      --port <num>                    Port for reciving the data from muliti UDP" << std::endl;
        std::cout << "                                      (Default: 51213)"                           << std::endl;
        std::cout << ""                                                                                 << std::endl;
            ...
            char* argv[])
    {
        ...
        for (int i = 2; i < argc; ++i)
        {
            ...
            else if (arg == "--groupip" || arg == "--groupIP")
            {
                if (config.entity == CLIParser::EntityKind::PUBLISHER)
                {
                    if (++i < argc)
                    {
                        config.pub_config.groupIP = argv[i];
                    }
                    else
                    {
                        EPROSIMA_LOG_ERROR(CLI_PARSER, "parsing group-ip argument");
                        print_help(EXIT_FAILURE);
                    }
                }
                else
                {
                    EPROSIMA_LOG_ERROR(CLI_PARSER, "group-ip argument is only valid for publisher entity");
                    print_help(EXIT_FAILURE);
                }
            }

            else if (arg == "--udpPort" || arg == "--udpPORT")
            {
                if (config.entity == CLIParser::EntityKind::PUBLISHER)
                {
                    if (++i < argc)
                    {
                        config.pub_config.port = std::stoi(argv[i]);
                    }
                    else
                    {
                ...
                {
                    EPROSIMA_LOG_ERROR(CLI_PARSER, "udpPORT argument is only valid for publisher entity");
                    print_help(EXIT_FAILURE);
                }
            }
